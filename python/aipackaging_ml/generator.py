...
def _has_hole(cells: tuple[tuple[int, int], ...]) -> bool:
    ...
    while queue:
        ...
        if column < -1 or row < -1 or column > width or row > height:
            continue
        outside.add(cell)
        queue.extend(((column - 1, row), (column + 1, row), (column, row - 1), (column, row + 1)))
...
def generate_problem(master_seed: int, tier: str, split: str, index: int, attempt: int = 0) -> dict[str, Any]:
    ...
    areas = [rng.randint(max(profile.part_area_min, int(minimum_average)), profile.part_area_max) for _ in range(type_count)]
