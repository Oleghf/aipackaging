...
После U1 проверка модели также отделена от прикладного потока. Контроллер зависит
от `IPolygonModelJobRunner`, а `StdThreadPolygonModelJobRunner` выполняет загрузку
через шлюз инфраструктуры и доставляет результат диспетчером приложения.
Идентификатор работы защищает от запоздалых событий; новая модель публикуется
только после полной проверки, поэтому ошибка не уничтожает прежнюю модель.

Снимок рабочей области содержит только нейтральные сведения о листе, деталях,
документе и модели. Qt самостоятельно владеет начальной страницей, недавними
файлами, пользовательскими режимами и `QSettings`. Соответствие режимов
прикладным запросам фиксировано тестами и не меняет поисковые контракты.
