...
- GUI-модель не использует полигональные отверстия, единицы, kerf, зазор и допуск
  кривых; эти возможности пока доступны только headless solver.
...
ни к клеточной policy, ни к полигональному solver; C++ ONNX inference входит в
M5. Текущая клеточная модель несовместима с динамическим polygon action space и
