...
полигонального раскроя: M2 подготавливает данные, но ещё не обучает нейросеть.
...
и отсутствие обученной модели. Текущий executable нельзя описывать как готовый к
промышленному раскрою произвольных контуров.
...
python/aipackaging_ml/ Python API, генератор и проверка датасета
