...
void replaceFile(const std::filesystem::path & temporary, const std::filesystem::path & destination, bool exists)
{
  ...
  if (!replaced)
    throw std::system_error(static_cast<int>(GetLastError()), std::system_category(), "не удалось заменить файл решения");
#else
  (void)exists;
                         ...
                         const std::function<void(AtomicWriteStage)> & fault)
{
  ...
  try
  {
    ...
    if (statusError && statusError != std::errc::no_such_file_or_directory)
      throw std::filesystem::filesystem_error("не удалось проверить файл решения", destination, statusError);
    if (exists && !std::filesystem::is_regular_file(status))
      throw std::invalid_argument("назначение решения должно быть обычным файлом");
    ...
    if (!output)
      throw std::runtime_error("не удалось открыть временный файл решения");
    const auto middle = content.size() / 2;
    output.write(content.data(), static_cast<std::streamsize>(middle));
    if (!output)
      throw std::runtime_error("не удалось записать временный файл решения");
    if (fault)
    ...
    output.close();
    if (!output)
      throw std::runtime_error("не удалось закрыть временный файл решения");
    if (fault)
      fault(AtomicWriteStage::BeforeReplace);
    replaceFile(temporary, destination, exists);
    error.clear();
    return true;
  }
